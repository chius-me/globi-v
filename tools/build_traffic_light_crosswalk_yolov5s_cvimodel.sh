...
HOST_WORK="$REPO_ROOT/duo-tpu/workspace/traffic_light_crosswalk_yolov5s"
MODEL_NAME="traffic_light_crosswalk_yolov5s"
...
nc: 6
names: [red, yellow, green, off, crosswalk, guide_arrows]
YAML
...
set -euo pipefail
cd /workspace/traffic_light_crosswalk_yolov5s
...
mkdir -p /workspace/traffic_light_crosswalk_yolov5s/work
cd /workspace/traffic_light_crosswalk_yolov5s/work
...
run_calibration.py ${MODEL_NAME}.mlir \
  --dataset /workspace/traffic_light_crosswalk_yolov5s/calib \
  --input_num 200 \
