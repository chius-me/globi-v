#!/usr/bin/env python3
import argparse
import json
import pathlib
import subprocess
import sys
from dataclasses import dataclass
from typing import Dict, List, Sequence, Tuple

import cv2
import numpy as np


@dataclass
class PrepInfo:
    orig_w: int
    orig_h: int
    resized_w: int
    resized_h: int
    paste_x: int
    paste_y: int


def sigmoid(x):
    return 1.0 / (1.0 + np.exp(-x))


def letterbox_bgr(image: np.ndarray, size: int = 640) -> Tuple[np.ndarray, PrepInfo]:
    h, w = image.shape[:2]
    ratio = min(size / w, size / h)
    resized_w = max(1, int(round(w * ratio)))
    resized_h = max(1, int(round(h * ratio)))
    resized = cv2.resize(image, (resized_w, resized_h), interpolation=cv2.INTER_LINEAR)
    canvas = np.zeros((size, size, 3), dtype=np.uint8)
    paste_x = (size - resized_w) // 2
    paste_y = (size - resized_h) // 2
    canvas[paste_y:paste_y + resized_h, paste_x:paste_x + resized_w] = resized
    return canvas, PrepInfo(w, h, resized_w, resized_h, paste_x, paste_y)


def to_input(image_bgr: np.ndarray) -> Tuple[np.ndarray, PrepInfo]:
    prepared, prep = letterbox_bgr(image_bgr, 640)
    rgb = cv2.cvtColor(prepared, cv2.COLOR_BGR2RGB)
    nchw = rgb.transpose(2, 0, 1).astype(np.float32) / 255.0
    return nchw[None, ...], prep


def restore_box(box: Sequence[float], prep: PrepInfo) -> List[float]:
    x1, y1, x2, y2 = box
    sx = prep.orig_w / prep.resized_w
    sy = prep.orig_h / prep.resized_h
    return [
        float(np.clip((x1 - prep.paste_x) * sx, 0, prep.orig_w - 1)),
        float(np.clip((y1 - prep.paste_y) * sy, 0, prep.orig_h - 1)),
        float(np.clip((x2 - prep.paste_x) * sx, 0, prep.orig_w - 1)),
        float(np.clip((y2 - prep.paste_y) * sy, 0, prep.orig_h - 1)),
    ]


def box_iou(a: np.ndarray, b: np.ndarray) -> np.ndarray:
    inter_x1 = np.maximum(a[0], b[:, 0])
    inter_y1 = np.maximum(a[1], b[:, 1])
    inter_x2 = np.minimum(a[2], b[:, 2])
    inter_y2 = np.minimum(a[3], b[:, 3])
    inter = np.maximum(0.0, inter_x2 - inter_x1) * np.maximum(0.0, inter_y2 - inter_y1)
    area_a = max(0.0, a[2] - a[0]) * max(0.0, a[3] - a[1])
    area_b = np.maximum(0.0, b[:, 2] - b[:, 0]) * np.maximum(0.0, b[:, 3] - b[:, 1])
    denom = area_a + area_b - inter
    return np.where(denom > 0.0, inter / denom, 0.0)


def nms(dets: List[Dict], num_classes: int, iou_thres: float, max_det: int) -> List[Dict]:
    final: List[Dict] = []
    for class_id in range(num_classes):
        candidates = [d for d in dets if d["class_id"] == class_id]
        candidates.sort(key=lambda d: d["confidence"], reverse=True)
        while candidates:
            best = candidates.pop(0)
            final.append(best)
            if not candidates:
                break
            boxes = np.array([d["xyxy_640"] for d in candidates], dtype=np.float32)
            ious = box_iou(np.array(best["xyxy_640"], dtype=np.float32), boxes)
            candidates = [d for d, iou in zip(candidates, ious) if iou <= iou_thres]
    final.sort(key=lambda d: d["confidence"], reverse=True)
    return final[:max_det]


def finalize(dets: List[Dict], names: Sequence[str], prep: PrepInfo, iou: float, max_det: int) -> List[Dict]:
    out = nms(dets, len(names), iou, max_det)
    for det in out:
        det["class_name"] = names[det["class_id"]]
        det["xyxy"] = restore_box(det.pop("xyxy_640"), prep)
    return out


def decode_pt(pred: np.ndarray, names: Sequence[str], prep: PrepInfo,
              conf: float, iou: float, max_det: int) -> List[Dict]:
    dets: List[Dict] = []
    pred = pred[0]
    obj = pred[:, 4]
    cls = pred[:, 5:5 + len(names)]
    scores = obj[:, None] * cls
    best_cls = np.argmax(scores, axis=1)
    best_conf = np.max(scores, axis=1)
    for idx in np.where(best_conf >= conf)[0].tolist():
        cx, cy, w, h = pred[idx, :4].tolist()
        dets.append({
            "class_id": int(best_cls[idx]),
            "confidence": float(best_conf[idx]),
            "xyxy_640": [cx - w * 0.5, cy - h * 0.5, cx + w * 0.5, cy + h * 0.5],
        })
    return finalize(dets, names, prep, iou, max_det)


def decode_onnx_split(outputs: Sequence[np.ndarray], names: Sequence[str], anchors: Sequence[Sequence[Tuple[float, float]]],
                      strides: Sequence[int], prep: PrepInfo, conf: float, iou: float, max_det: int) -> List[Dict]:
    dets: List[Dict] = []
    for scale_idx in range(3):
        bbox = outputs[scale_idx * 3 + 0]
        obj = outputs[scale_idx * 3 + 1]
        cls = outputs[scale_idx * 3 + 2]
        stride = strides[scale_idx]
        for anchor_idx in range(3):
            obj_s = sigmoid(obj[anchor_idx, :, :, 0])
            cls_s = sigmoid(cls[anchor_idx])
            scores = obj_s[:, :, None] * cls_s
            best_cls = np.argmax(scores, axis=2)
            best_conf = np.max(scores, axis=2)
            ys, xs = np.where(best_conf >= conf)
            for y, x in zip(ys.tolist(), xs.tolist()):
                raw = bbox[anchor_idx, y, x]
                cx = (float(sigmoid(raw[0])) * 2.0 - 0.5 + x) * stride
                cy = (float(sigmoid(raw[1])) * 2.0 - 0.5 + y) * stride
                bw = (float(sigmoid(raw[2])) * 2.0) ** 2 * anchors[scale_idx][anchor_idx][0]
                bh = (float(sigmoid(raw[3])) * 2.0) ** 2 * anchors[scale_idx][anchor_idx][1]
                dets.append({
                    "class_id": int(best_cls[y, x]),
                    "confidence": float(best_conf[y, x]),
                    "xyxy_640": [cx - bw * 0.5, cy - bh * 0.5, cx + bw * 0.5, cy + bh * 0.5],
                })
    return finalize(dets, names, prep, iou, max_det)


def color_for(class_id: int) -> Tuple[int, int, int]:
    palette = [(0, 0, 255), (0, 215, 255), (0, 200, 0), (180, 180, 180)]
    return palette[class_id % len(palette)]


def draw(image: np.ndarray, detections: Sequence[Dict]) -> np.ndarray:
    out = image.copy()
    for det in detections:
        x1, y1, x2, y2 = [int(round(v)) for v in det["xyxy"]]
        color = color_for(det["class_id"])
        cv2.rectangle(out, (x1, y1), (x2, y2), color, 2)
        label = f'{det["class_name"]}:{det["confidence"]:.2f}'
        cv2.putText(out, label, (x1, max(16, y1 - 5)), cv2.FONT_HERSHEY_SIMPLEX, 0.55, color, 2)
    return out


def load_pt_model(weights: pathlib.Path, yolov5_dir: pathlib.Path):
    import pathlib as pathlib_module
    import torch

    pathlib_module.WindowsPath = pathlib_module.PosixPath
    sys.path.insert(0, str(yolov5_dir))
    ckpt = torch.load(str(weights), map_location="cpu", weights_only=False)
    model = (ckpt.get("ema") or ckpt["model"]).float().eval()
    raw_names = model.names
    names = [raw_names[i] for i in range(len(raw_names))] if isinstance(raw_names, dict) else list(raw_names)
    strides = [int(v) for v in model.stride.detach().cpu().numpy().tolist()]
    anchors_grid = model.model[-1].anchors.detach().cpu().numpy().astype(float)
    for idx, stride in enumerate(strides):
        anchors_grid[idx] *= stride
    anchors = anchors_grid.tolist()
    return model, names, anchors, strides


def transcode(tmp_path: pathlib.Path, source_video: pathlib.Path, out_path: pathlib.Path) -> None:
    subprocess.run([
        "ffmpeg", "-y", "-hide_banner", "-loglevel", "error",
        "-i", str(tmp_path),
        "-i", str(source_video),
        "-map", "0:v:0",
        "-map", "1:a?",
        "-c:v", "libx264",
        "-preset", "veryfast",
        "-crf", "20",
        "-pix_fmt", "yuv420p",
        "-c:a", "copy",
        str(out_path),
    ], check=True)
    tmp_path.unlink(missing_ok=True)


def annotate_video(video: pathlib.Path, out_path: pathlib.Path, mode: str, args, pt_model, names, anchors, strides,
                   onnx_session=None, onnx_input_name=None):
    import torch

    cap = cv2.VideoCapture(str(video))
    if not cap.isOpened():
        raise RuntimeError(f"failed to open video: {video}")
    fps = cap.get(cv2.CAP_PROP_FPS) or 25.0
    width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH) or 0)
    height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT) or 0)
    total = int(cap.get(cv2.CAP_PROP_FRAME_COUNT) or 0)
    tmp_path = out_path.with_suffix(".raw.mp4")
    writer = cv2.VideoWriter(str(tmp_path), cv2.VideoWriter_fourcc(*"mp4v"), fps, (width, height))
    frame_idx = 0
    frames_with_det = 0
    total_det = 0
    by_class = {name: 0 for name in names}
    while True:
        ok, frame = cap.read()
        if not ok or frame is None:
            break
        inp, prep = to_input(frame)
        if mode == "pt":
            with torch.no_grad():
                pred = pt_model(torch.from_numpy(inp))[0].detach().cpu().numpy()
            dets = decode_pt(pred, names, prep, args.conf, args.iou, args.max_det)
        else:
            outputs = onnx_session.run(None, {onnx_input_name: inp})
            dets = decode_onnx_split(outputs, names, anchors, strides, prep, args.conf, args.iou, args.max_det)
        writer.write(draw(frame, dets))
        if dets:
            frames_with_det += 1
        total_det += len(dets)
        for det in dets:
            by_class[det["class_name"]] += 1
        frame_idx += 1
        if frame_idx % 100 == 0:
            print(f"{mode} {video.name}: {frame_idx}/{total or '?'} det={total_det}")
    writer.release()
    cap.release()
    transcode(tmp_path, video, out_path)
    return {
        "mode": mode,
        "video": str(video),
        "out": str(out_path),
        "frames": frame_idx,
        "frames_with_det": frames_with_det,
        "detections": total_det,
        "by_class": by_class,
    }


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--video", required=True)
    parser.add_argument("--pt", required=True)
    parser.add_argument("--onnx", required=True)
    parser.add_argument("--yolov5-dir", default="third_party/yolov5")
    parser.add_argument("--out", required=True)
    parser.add_argument("--conf", type=float, default=0.10)
    parser.add_argument("--iou", type=float, default=0.50)
    parser.add_argument("--max-det", type=int, default=100)
    args = parser.parse_args()

    import onnxruntime as ort

    out_dir = pathlib.Path(args.out)
    out_dir.mkdir(parents=True, exist_ok=True)
    video = pathlib.Path(args.video)
    pt_model, names, anchors, strides = load_pt_model(pathlib.Path(args.pt), pathlib.Path(args.yolov5_dir))
    print("names", names)
    print("strides", strides)
    print("anchors", anchors)
    session = ort.InferenceSession(args.onnx, providers=["CPUExecutionProvider"])
    input_name = session.get_inputs()[0].name
    tag = f"conf{int(args.conf * 100):03d}"
    summaries = [
        annotate_video(video, out_dir / f"{video.stem}_pt_{tag}.mp4", "pt", args, pt_model, names, anchors, strides),
        annotate_video(video, out_dir / f"{video.stem}_onnx_{tag}.mp4", "onnx", args, pt_model, names, anchors, strides, session, input_name),
    ]
    (out_dir / f"summary_{tag}.json").write_text(json.dumps(summaries, indent=2, ensure_ascii=False), encoding="utf-8")
    print(json.dumps(summaries, indent=2, ensure_ascii=False))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
